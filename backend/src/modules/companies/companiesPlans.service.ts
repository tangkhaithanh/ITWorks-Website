...
export class CompaniesPlansService {
    ...
    private async getCompanyIdByAccount(accountId: bigint, tx: Tx = this.prisma): Promise<bigint> {
        try {
            ...
            if (!company) throw new NotFoundException('Tài khoản chưa liên kết với thông tin công ty.');
            return company.id;
        } catch (error) {
            if (error instanceof NotFoundException) throw error;
            console.error('Error getCompanyIdByAccount:', error);
            throw new InternalServerErrorException('Lỗi hệ thống khi lấy ID công ty.');
        }
    }
    ...
    async assignPlanStackQuotaTx(
        ...
        if (orderId) {
            const exists = await tx.companyPlan.findFirst({ where: { order_id: orderId } });
            if (exists) throw new ConflictException('Đơn hàng này đã được kích hoạt gói dịch vụ rồi.');

            const existsHistory = await tx.companyPlanHistory.findFirst({ where: { order_id: orderId } });
            if (existsHistory) throw new ConflictException('Đơn hàng này đã nằm trong lịch sử giao dịch.');
        }
        ...
        const currentPlan = await tx.companyPlan.findUnique({ where: { company_id: companyId } });
        const now = new Date();
        ...
        if (currentPlan) {
            const isActive = currentPlan.status === 'active' && currentPlan.end_date > now;
            ...
            const creditsUsed = currentPlan.credit_amount_snapshot - currentPlan.credits_left;
        ...
        const newEndDate = new Date(now.getTime() + newPlan.duration_days * 24 * 60 * 60 * 1000);
    ...
    // Wrapper Transaction
    async assignPlanStackQuota(accountId: bigint, newPlanId: bigint, orderId?: bigint) {
        try {
            ...
            throw new InternalServerErrorException('Lỗi hệ thống khi kích hoạt gói dịch vụ.');
        }
    }
        ...
        try {
            ...
            return await this.prisma.$transaction(async (tx) => {
                ...
                if (!current || current.status !== 'active' || current.end_date <= now) {
                    throw new ForbiddenException('Bạn chưa có gói dịch vụ hoặc gói đã hết hạn.');
                }
                ...
                if (result.count === 0) {
                    throw new ForbiddenException('Đã hết lượt đăng tin (Quota exhausted). Vui lòng nâng cấp gói.');
                }
            ...
            throw new InternalServerErrorException('Lỗi hệ thống khi trừ lượt đăng tin.');
        }
    }
    ...
    async getCurrentPlanSummary(accountId: bigint) {
        try {
            ...
            const current = await this.prisma.companyPlan.findFirst({
                where: { company_id: companyId, status: 'active', end_date: { gt: now } },
                include: { plan: true }
            });
