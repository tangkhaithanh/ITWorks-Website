...
export class ElasticsearchJobService {
  ...
  private mapJob(job: any) {
    ...
    workModes.forEach(
      (m) => typeof m === 'string' && m.trim() && inputs.add(m.trim()),
    );
    experienceLevels.forEach(
      (e) => typeof e === 'string' && e.trim() && inputs.add(e.trim()),
    );
    employmentTypes.forEach(
      (t) => typeof t === 'string' && t.trim() && inputs.add(t.trim()),
    );
